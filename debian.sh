...
myuser=byvoid
mygroup=byvoid
wherepubkey="edit your pubkey URL here"
sudopasswd=1
sshdport=6070
iftrim=0
grubtimeout=0
...
echo "group not add because group byvoid already exists!!!!!!"
echo "group not add because group byvoid already exists!" >> configerror.log
exit 1
fi
...
echo "group not add because user byvoid already exists!!!!!!"
echo "group not add because user byvoid already exists!" >> configerror.log
exit 1
fi
...
# 下载中科大源模板，并去掉其他源
wget -O /tmp/ustcsources.txt https://mirrors.ustc.edu.cn/repogen/conf/debian-https-4-stretch
...
mkdir -p /root/.ssh
wget -O /tmp/pubkey $wherepubkey
mv /tmp/pubkey /root/.ssh/authorized_keys
